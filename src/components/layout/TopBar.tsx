    ...
    <div className="lt-topbar">
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ position: "relative" }} role="search" onClick={function (e) { e.stopPropagation(); }}>
          ...
          {!localGs ? (
            <span aria-hidden="true" className="lt-hide-mobile" style={{ position: "absolute" as const, right: 10, top: 9, fontSize: 10, fontFamily: "var(--font-mono)", color: "var(--text-muted)", background: "var(--bg-primary)", border: "1px solid var(--border)", borderRadius: 6, padding: "2px 6px", lineHeight: 1.2, fontWeight: 500 }}>
        ...
        {(critCount > 0 || urgences.length > 0) ? (
          <div style={{ position: "relative" }} onClick={function (e) { e.stopPropagation(); }}>
            <button
              ...
              style={{ background: critCount > 0 ? "var(--danger-bg)" : "var(--warning-bg)", color: critCount > 0 ? "var(--danger)" : "var(--warning)", borderRadius: 999, padding: "4px 10px", fontSize: 12, fontWeight: 600, border: "1px solid " + (critCount > 0 ? "var(--danger-border)" : "var(--warning-border)"), cursor: "pointer" }}
            >
            ...
            {urgOpen ? (
              <div style={{ position: "absolute", top: 36, right: 0, width: 340, maxWidth: "90vw", background: "var(--bg-primary)", borderRadius: 12, boxShadow: "0 8px 30px var(--shadow-lg)", border: "1px solid var(--border)", zIndex: 200, maxHeight: 400, overflow: "auto" }}>
                <div style={{ padding: "10px 14px", fontSize: 11, fontWeight: 700, color: "var(--text-secondary)", background: "var(--bg-tertiary)", borderBottom: "1px solid var(--border)", textTransform: "uppercase", letterSpacing: 0.5, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ background: critCount > 0 ? "var(--danger)" : "var(--warning)", color: "white", borderRadius: 999, padding: "1px 7px", fontSize: 11 }}>{String(urgences.length)}</span>
                </div>
                {urgences.length === 0 ? (
                  ...
                  return (
                    <div key={i} onClick={function () { setMl({ t: "det", did: u.did }); setUrgOpen(false); }} style={{ padding: "10px 14px", borderBottom: "1px solid var(--border-light)", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 700, background: u.level === "critical" ? "var(--danger-light)" : "var(--warning-bg)", color: u.level === "critical" ? "var(--danger)" : "var(--warning)", padding: "2px 6px", borderRadius: 6, flexShrink: 0, marginTop: 2 }}>{u.level === "critical" ? "URGENT" : "ALERTE"}</span>
                    </div>
                  );
                })}
              </div>
