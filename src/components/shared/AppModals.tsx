...
function AppModals(p: AppModalsProps) {
  ...
  return (
    <>
      {ml && ml.t === "ndos" ? <Overlay close={function () { setMl(null); }} title="Nouveau dossier" w={820}><NDosForm allDos={dos} members={(p.teamProps || {}).members || []} onSave={addDos} onClose={function () { setMl(null); }} nf={nf} scan={ml.scan || null} apiKey={cfg.geminiKey || ""} setMl={setMl} /></Overlay> : null}
      ...
      {ml && ml.t === "det" ? <Overlay close={function () { setMl(null); }} title="Détails" w={820}><ErrorBound onClose={function () { setMl(null); }}><DetView did={ml.did} dos={dos} tcs={tcs} dep={dep} logs={logs} cfg={cfg} advance={advance} setAdvPending={setAdvPending} updateTcDate={updateTcDate} deleteTc={p.deleteTc} editTcInfo={p.editTcInfo} updateGarantie={updateGarantie} setMl={setMl} dispatch={dispatch} deleteDos={deleteDos} closeDos={closeDos} archiveDos={archiveDos} setDosSt={setDosSt} toggleDepSt={toggleDepSt} patchDos={patchDos} syncDPWorld={syncDPWorld} syncCMA={syncCMA} syncCarrier={p.syncCarrier} humanPhrase={humanPhrase} sv={sv} db={db} canEdit={canEdit} shareTracking={shareTracking} companyId={companyId} nf={nf} /></ErrorBound></Overlay> : null}
      ...
      {ml && ml.t === "scan" ? <Overlay close={function () { setMl(null); }} title="Scanner un BL" w={520}><ScanBL apiKey={cfg.geminiKey || ""} onResult={function (r) { setMl({ t: "ndos", scan: r }); }} /></Overlay> : null}
      ...
      {advPending ? <div onClick={function () { setAdvPending(null); }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.35)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div onClick={function (e) { e.stopPropagation(); }} style={{ background: "var(--bg-primary)", borderRadius: 14, padding: 24, width: 340, maxWidth: "90vw", boxShadow: "var(--shadow-lg)" }}>
