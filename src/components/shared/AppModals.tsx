...
function AppModals(p: AppModalsProps) {
  ...
  return (
    <>
      ...
      {ml && ml.t === "det" ? <Overlay close={function () { setMl(null); }} title="Détails" w={820}><ErrorBound onClose={function () { setMl(null); }}><DetView did={ml.did} dos={dos} tcs={tcs} dep={dep} logs={logs} cfg={cfg} advance={advance} setAdvPending={setAdvPending} updateTcDate={updateTcDate} deleteTc={p.deleteTc} editTcInfo={p.editTcInfo} updateGarantie={updateGarantie} setMl={setMl} dispatch={dispatch} deleteDos={deleteDos} closeDos={closeDos} archiveDos={archiveDos} setDosSt={setDosSt} toggleDepSt={toggleDepSt} patchDos={patchDos} syncDPWorld={syncDPWorld} syncCMA={syncCMA} humanPhrase={humanPhrase} sv={sv} db={db} canEdit={canEdit} shareTracking={shareTracking} companyId={companyId} nf={nf} /></ErrorBound></Overlay> : null}
