    ...
    <div>
      ...
      <div style={{ background: "linear-gradient(135deg,#1c1917,#292524)", padding: 20, borderRadius: 12, color: "white", marginBottom: 14, position: "relative" }}>
        ...
        {dm ? <div role="menu" style={{ position: "absolute", right: 12, top: 52, background: "var(--bg-primary)", border: "1px solid var(--border)", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,.15)", zIndex: 50, minWidth: 180, overflow: "hidden" }}>
          ...
          {/* Sync armateur generique : detecte automatiquement (CMA / Maersk / MSC / Hapag / ONE / Grimaldi) */}
          {p.syncCarrier ? (
            <ClickableDiv onClick={function () { setDm(false); p.syncCarrier(d.id); }} label="Synchroniser avec l'armateur (date d'arrivee, n\u00B0 TC)" style={{ padding: "10px 14px", fontSize: 13, display: "flex", alignItems: "center", gap: 8, color: "var(--text-input)", borderBottom: "1px solid var(--border-light)" }}><span>{"\uD83C\uDF10"}</span>{"Sync armateur"}</ClickableDiv>
          ) : null}
